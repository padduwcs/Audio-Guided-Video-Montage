...
Schema, field, mapping: [`02_data_contract.md`](./02_data_contract.md) · module I/O: [`01_system_architecture.md` §4–5](./01_system_architecture.md).

| File | Tạo bởi | Điều kiện tích hợp tối thiểu |
| ---- | ------- | ---------------------------- |
| `media_metadata.json` | `input_processor/` | Parse OK; ≥1 video usable; audio usable; `normalized_path` tồn tại |
| `audio_segments.json` | `audio_analyzer/` | ≥1 segment; `segment_id` unique; `query` không rỗng |
| `clip_metadata.json` | `video_analyzer/` | ≥1 clip usable; `clip_id` unique; `video_id` khớp metadata |
| `embedding_metadata.json` | `embedding_indexer/` | Map đúng segment/clip; index path OK |
| `matching_candidates.json` | `matching_engine/` | ID khớp upstream; `final_score` ∈ [0,1] |
| `timeline.json` | `timeline_planner/` / `review_ui/` | Timing hợp lệ; `source_path` tồn tại; tổng ≈ audio duration |
