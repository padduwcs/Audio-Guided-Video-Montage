...
Trước khi code, mỗi thành viên cần đọc các tài liệu liên quan trong `docs/`.
...
* Tính semantic score.
* Kết hợp quality score, duration fit, diversity, continuity nếu có.
* Trả về top-k clip.
* Gán confidence.
...
Thư mục `schemas/` mô tả chi tiết schema của từng file dữ liệu quan trọng.
...
Thư mục `samples/` chứa các file JSON mẫu đúng Data Contract.
...
* Người làm UI có thể dùng `timeline_sample.json` và `matching_candidates_sample.json` để dựng giao diện mà không cần chờ Matching Engine hoàn thành.
* Người làm renderer có thể dùng `timeline_sample.json` để test render video mà không cần chờ toàn bộ pipeline.
* Người làm matching có thể nhìn `matching_candidates_sample.json` để biết output cần xuất ra.
...
6. File stage tương ứng với module mình phụ trách
...
1. Đọc Product Scope.
2. Đọc System Architecture.
3. Đọc Data Contract.
4. Đọc stage spec của module mình.
...
Trạng thái hiện tại của bộ tài liệu:

```text
problem.md                         Đã có
analysis.md                        Đã có
details/00_project_scope.md         Cần chốt / cập nhật
details/01_system_architecture.md   Cần chốt / cập nhật
details/02_data_contract.md         Cần chốt / cập nhật
details/03_stage_1_input_processing.md      Chưa viết
details/04_stage_2_audio_analysis.md        Chưa viết
details/05_stage_3_video_analysis.md        Chưa viết
details/06_stage_4_embedding_indexing.md    Chưa viết
details/07_stage_5_matching_engine.md       Chưa viết
details/08_stage_6_timeline_planning.md     Chưa viết
details/09_stage_7_review_ui.md             Chưa viết
details/10_stage_8_rendering.md             Chưa viết
details/11_team_assignment.md               Chưa viết
details/12_integration_plan.md              Chưa viết
schemas/                                    Chưa hoàn thiện
samples/                                    Chưa hoàn thiện
```

Khi tài liệu nào được chốt, cần cập nhật trạng thái để các thành viên biết đâu là bản có thể dùng để triển khai.
