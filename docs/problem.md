...
Đầu ra bắt buộc là một **video hoàn chỉnh duy nhất**, trong đó các phân cảnh được trích lọc từ tập video đầu vào, sắp xếp và đồng bộ hóa thời lượng sao cho hình ảnh phù hợp nhất có thể về mặt ngữ nghĩa và nhịp độ với nội dung của lời thuyết minh.
...
* **Video hoàn chỉnh ($V'$):** Sản phẩm đa phương tiện duy nhất đã được kết xuất (render). $V'$ phải thỏa mãn đồng thời các hệ ràng buộc cốt lõi sau:
  ...
  * ⏱️ **Ràng buộc về thời gian (Temporal Synchronization):** Điểm bắt đầu, điểm kết thúc và nhịp độ của các phân cảnh hình ảnh phải được tính toán và căn chỉnh linh hoạt (thông qua các thao tác như cắt xén, tua nhanh, làm chậm, hoặc giữ khung hình) để bám sát trường độ của lời dẫn trong $A$ trong phạm vi footage có sẵn.
